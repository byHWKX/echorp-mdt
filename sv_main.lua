...
AddEventHandler('erp_mdt:AddLog', function(text)
	exports.oxmysql:fetchSync('INSERT INTO `pd_logs` (`text`, `time`) VALUES (:text, :time)', { text = text, time = os.time() * 1000 })
end)

local function GetNameFromId(cid, cb)
	cb(exports.oxmysql:fetchSync('SELECT firstname, lastname FROM `users` WHERE id = :id LIMIT 1', { id = cid }))
end
...
AddEventHandler('echorp:doLogout', function(sentData) 
	knownradios[sentData['cid']] = nil
end)

AddEventHandler('echorp:playerDropped', function(playerInfo)
	if playerInfo then
		knownradios[playerInfo.cid] = nil
		if playerInfo.job.name == "ambulance" then
			TriggerEvent('echorp:notifyDuty',playerInfo,"drop")
		elseif playerInfo.isPolice then
			TriggerEvent('echorp:notifyDuty',playerInfo,"drop")
		end
	end
end)

RegisterNetEvent('erp-mdt:setRadio')
AddEventHandler('erp-mdt:setRadio', function(sentRadio) 
	TriggerEvent('echorp:getplayerfromid', source, function(result)
		if result then
			if tonumber(sentRadio) < 100 then
				knownradios[result['cid']] = tonumber(sentRadio)
			else
				knownradios[result['cid']] = 'hidden'
			end
		end
	end)
end)
	...
	TriggerEvent('echorp:getplayerfromid', source, function(result)
		if result then
			if result.job and (result.job.isPolice or (result.job.name == 'ambulance' or result.job.name == 'doj')) then
				...
				TriggerClientEvent('erp_mdt:getActiveUnits', source, lspd, bcso, sahp, sasp, doc, sapr, pa, ems)
			end
		end
	end)
end)
...
local function GetIncidentName(id, cb)
	cb(exports.oxmysql:fetchSync('SELECT title FROM `pd_incidents` WHERE id = :id LIMIT 1', { id = id }))
end
	...
	TriggerEvent('echorp:getplayerfromid', source, function(result)
		if result then
			if result.job and (result.job.isPolice or result.job.name == 'doj') then
				-- get warrants
				exports.oxmysql:fetch("SELECT * FROM pd_convictions WHERE warrant='1'", {}, function(warrants)
					for i=1, #warrants do
			...
			elseif result.job and (result.job.name == 'ambulance') then
				exports.oxmysql:fetch("SELECT * FROM `ems_reports` ORDER BY `id` DESC LIMIT 20", {}, function(matches)
...
local function GetConvictions(cid, cb)
	cb((exports.oxmysql:fetchSync('SELECT * FROM `pd_convictions` WHERE `cid`=:cid', { cid = cid })))
end

local function GetLicenseInfo(cid, cb)
	cb(exports.oxmysql:fetchSync('SELECT * FROM `licenses` WHERE `cid`=:cid', { cid = cid }))
end
...
AddEventHandler('erp_mdt:searchProfile', function(sentData)
	if sentData then
		...
		TriggerEvent('echorp:getplayerfromid', source, function(result)
			if result then
				if result.job and (result.job.isPolice or result.job.name == 'doj') then
					exports.oxmysql:fetch("SELECT id, firstname, lastname, gender, profilepic FROM `users` WHERE LOWER(`firstname`) LIKE :query OR LOWER(`lastname`) LIKE :query OR LOWER(`id`) LIKE :query OR CONCAT(LOWER(`firstname`), ' ', LOWER(`lastname`)) LIKE :query LIMIT 20", {
				...
				elseif result.job and (result.job.name == 'ambulance') then
					exports.oxmysql:fetch("SELECT id, firstname, lastname, gender, profilepic, dateofbirth FROM `users` WHERE LOWER(`firstname`) LIKE :query OR LOWER(`lastname`) LIKE :query OR LOWER(`id`) LIKE :query OR CONCAT(LOWER(`firstname`), ' ', LOWER(`lastname`)) LIKE :query LIMIT 20", {
						query = string.lower('%'..sentData..'%')
					}, function(people)
						for i=1, #people do
							people[i]['warrant'] = false
	...
	TriggerEvent('echorp:getplayerfromid', source, function(result)
		if result then
			if result.job and result.job.isPolice then
				exports.oxmysql:fetch('SELECT * FROM `pd_bulletin`', {}, function(bulletin)
					TriggerClientEvent('erp_mdt:dashboardbulletin', result.source, bulletin)
				end)
			elseif result.job and (result.job.name == 'ambulance') then
				exports.oxmysql:fetch('SELECT * FROM `ems_bulletin`', {}, function(bulletin)
					TriggerClientEvent('erp_mdt:dashboardbulletin', result.source, bulletin)
				end)
			elseif result.job and (result.job.name == 'doj') then
				exports.oxmysql:fetch('SELECT * FROM `doj_bulletin`', {}, function(bulletin)
					TriggerClientEvent('erp_mdt:dashboardbulletin', result.source, bulletin)
				end)
			end
		end
...
AddEventHandler('erp_mdt:deleteBulletin', function(id)
	if id then
		TriggerEvent('echorp:getplayerfromid', source, function(result)
			if result then
				if result.job and result.job.isPolice then
					exports.oxmysql:fetch('SELECT `title` FROM `pd_bulletin` WHERE id=:id LIMIT 1', { id = id}, function(res)
						if res and res[1] then
							exports.oxmysql:fetchSync("DELETE FROM `pd_bulletin` WHERE id=:id", { id = id })
				...
				elseif result.job and (result.job.name == 'ambulance') then
					exports.oxmysql:fetch('SELECT `title` FROM `ems_bulletin` WHERE id=:id LIMIT 1', { id = id}, function(res)
						if res and res[1] then
							exports.oxmysql:fetchSync("DELETE FROM `ems_bulletin` WHERE id=:id", { id = id })
							TriggerEvent('erp_mdt:AddLog', "A bulletin was deleted by "..result.firstname.." "..result.lastname.." with the title: "..res[1]['title'].."!")
							TriggerClientEvent('erp_mdt:deleteBulletin', -1, result.source, id, result.job.name)
						end
					end)
				elseif result.job and (result.job.name == 'doj') then
					exports.oxmysql:fetch('SELECT `title` FROM `doj_bulletin` WHERE id=:id LIMIT 1', { id = id}, function(res)
						if res and res[1] then
							exports.oxmysql:fetchSync("DELETE FROM `doj_bulletin` WHERE id=:id", { id = id })
							TriggerEvent('erp_mdt:AddLog', "A bulletin was deleted by "..result.firstname.." "..result.lastname.." with the title: "..res[1]['title'].."!")
							TriggerClientEvent('erp_mdt:deleteBulletin', -1, result.source, id, result.job.name)
						end
					end)
...
local function GetPersonInformation(cid, table, cb)
	cb(exports.oxmysql:fetchSync('SELECT information, tags, gallery FROM '..table..' WHERE cid=:cid', { cid = cid }))
end

local function GetVehicleInformation(cid, cb)
	cb(exports.oxmysql:fetchSync('SELECT id, plate, vehicle FROM owned_vehicles WHERE owner=:cid', { cid = cid }))
end
...
AddEventHandler('erp_mdt:getProfileData', function(sentId)
	...
	TriggerEvent('echorp:getplayerfromid', source, function(result)
		if result then
			if result.job and (result.job.isPolice or result.job.name == 'doj') then
				exports.oxmysql:fetch('SELECT id, firstname, lastname, job, profilepic, gender, dateofbirth FROM users WHERE id=:id LIMIT 1', { id = sentId }, function(user)
		...
		if result then
			if result.job and (result.job.isPolice or result.job.name == 'doj') then
				local function UpdateInfo(id, pfp, desc)
					exports.oxmysql:fetchSync("UPDATE policemdtdata SET `information`=:information WHERE `id`=:id LIMIT 1", { id = id, information = information })
					exports.oxmysql:fetchSync("UPDATE users SET `profilepic`=:profilepic WHERE `id`=:id LIMIT 1", { id = cid, profilepic = pfp })
					TriggerEvent('erp_mdt:AddLog', "A user with the Citizen ID "..cid.." was updated by "..result.fullname)

					if result.job.name == 'doj' then
						exports.oxmysql:fetchSync("UPDATE users SET `firstname`=:firstname, `lastname`=:lastname WHERE `id`=:id LIMIT 1", { firstname = fName, lastname = sName, id = cid })
					end
				end

				exports.oxmysql:fetch('SELECT id FROM policemdtdata WHERE cid=:cid LIMIT 1', { cid = cid }, function(user)
					if user and user[1] then
			...
			elseif result.job and (result.job.name == 'ambulance') then
				local function UpdateInfo(id, pfp, desc)
					exports.oxmysql:fetchSync("UPDATE emsmdtdata SET `information`=:information WHERE `id`=:id LIMIT 1", { id = id, information = information })
					exports.oxmysql:fetchSync("UPDATE users SET `profilepic`=:profilepic WHERE `id`=:id LIMIT 1", { id = cid, profilepic = pfp })
					TriggerEvent('erp_mdt:AddLog', "A user with the Citizen ID "..cid.." was updated by "..result.fullname)
				end

				exports.oxmysql:fetch('SELECT id FROM emsmdtdata WHERE cid=:cid LIMIT 1', { cid = cid }, function(user)
					if user and user[1] then
						UpdateInfo(user[1]['id'], pfp, information)
					else
		...
		if result then
			if result.job and (result.job.isPolice or result.job.name == 'doj') then
				local function UpdateTags(id, tags)
					exports.oxmysql:fetchSync("UPDATE policemdtdata SET `tags`=:tags WHERE `id`=:id LIMIT 1", { id = id, tags = json.encode(tags) })
					TriggerEvent('erp_mdt:AddLog', "A user with the Citizen ID "..id.." was added a new tag with the text ("..tag..") by "..result.fullname)
				end

				exports.oxmysql:fetch('SELECT id, tags FROM policemdtdata WHERE cid=:cid LIMIT 1', { cid = cid }, function(user)
					if user and user[1] then
			...
			elseif result.job and (result.job.name == 'ambulance') then
				local function UpdateTags(id, tags)
					exports.oxmysql:fetchSync("UPDATE emsmdtdata SET `tags`=:tags WHERE `id`=:id LIMIT 1", { id = id, tags = json.encode(tags) })
					TriggerEvent('erp_mdt:AddLog', "A user with the Citizen ID "..id.." was added a new tag with the text ("..tag..") by "..result.fullname)
				end

				exports.oxmysql:fetch('SELECT id, tags FROM emsmdtdata WHERE cid=:cid LIMIT 1', { cid = cid }, function(user)
					if user and user[1] then
						local tags = json.decode(user[1]['tags'])
						table.insert(tags, tag)
						UpdateTags(user[1]['id'], tags)
		...
		if result then
			if result.job and (result.job.isPolice or result.job.name == 'doj') then
				...
				local function UpdateTags(id, tag)
					exports.oxmysql:fetchSync("UPDATE policemdtdata SET `tags`=:tags WHERE `id`=:id LIMIT 1", { id = id, tags = json.encode(tag) })
			...
			elseif result.job and (result.job.name == 'ambulance') then
				...
				local function UpdateTags(id, tag)
					exports.oxmysql:fetchSync("UPDATE emsmdtdata SET `tags`=:tags WHERE `id`=:id LIMIT 1", { id = id, tags = json.encode(tag) })
					TriggerEvent('erp_mdt:AddLog', "A user with the Citizen ID "..id.." was removed of a tag with the text ("..tagtext..") by "..result.fullname)
				end
		...
		if result then
			if result.job and (result.job.isPolice or result.job.name == 'doj') then
				...
				local function UpdateGallery(id, gallery)
					exports.oxmysql:fetchSync("UPDATE policemdtdata SET `gallery`=:gallery WHERE `id`=:id LIMIT 1", { id = id, gallery = json.encode(gallery) })
					TriggerEvent('erp_mdt:AddLog', "A user with the Citizen ID "..id.." had their gallery updated (+) by "..result.fullname)
				end

				exports.oxmysql:fetch('SELECT id, gallery FROM policemdtdata WHERE cid=:cid LIMIT 1', { cid = cid }, function(user)
					if user and user[1] then
			...
			elseif result.job and (result.job.name == 'ambulance') then
				...
				local function UpdateGallery(id, gallery)
					exports.oxmysql:fetchSync("UPDATE emsmdtdata SET `gallery`=:gallery WHERE `id`=:id LIMIT 1", { id = id, gallery = json.encode(gallery) })
					TriggerEvent('erp_mdt:AddLog', "A user with the Citizen ID "..id.." had their gallery updated (+) by "..result.fullname)
				end

				exports.oxmysql:fetch('SELECT id, gallery FROM emsmdtdata WHERE cid=:cid LIMIT 1', { cid = cid }, function(user)
					if user and user[1] then
						local imgs = json.decode(user[1]['gallery'])
						table.insert(imgs, img)
						UpdateGallery(user[1]['id'], imgs)
...
AddEventHandler('erp_mdt:getAllIncidents', function()
	TriggerEvent('echorp:getplayerfromid', source, function(result)
		if result then
			if result.job and (result.job.isPolice or result.job.name == 'doj') then
				exports.oxmysql:fetch("SELECT * FROM `pd_incidents` ORDER BY `id` DESC LIMIT 30", {}, function(matches)
...
AddEventHandler('erp_mdt:searchIncidents', function(query)
	if query then
		TriggerEvent('echorp:getplayerfromid', source, function(result)
			if result then
				if result.job and (result.job.isPolice or result.job.name == 'doj') then
					exports.oxmysql:fetch("SELECT * FROM `pd_incidents` WHERE `id` LIKE :query OR LOWER(`title`) LIKE :query OR LOWER(`author`) LIKE :query OR LOWER(`details`) LIKE :query OR LOWER(`tags`) LIKE :query OR LOWER(`officersinvolved`) LIKE :query OR LOWER(`civsinvolved`) LIKE :query OR LOWER(`author`) LIKE :query ORDER BY `id` DESC LIMIT 50", {
...
AddEventHandler('erp_mdt:getIncidentData', function(sentId)
	if sentId then
		TriggerEvent('echorp:getplayerfromid', source, function(result)
			if result then
				if result.job and (result.job.isPolice or result.job.name == 'doj') then
					exports.oxmysql:fetch("SELECT * FROM `pd_incidents` WHERE `id` = :id", {
						id = sentId
					}, function(matches)
						...
						exports.oxmysql:fetch("SELECT * FROM `pd_incidents` WHERE `id` = :id", {
							...
							exports.oxmysql:fetch("SELECT * FROM `pd_convictions` WHERE `linkedincident` = :id", {
								id = sentId
...
AddEventHandler('erp_mdt:getAllBolos', function()
	TriggerEvent('echorp:getplayerfromid', source, function(result)
		if result then
			if result.job and (result.job.isPolice or result.job.name == 'doj') then
				exports.oxmysql:fetch("SELECT * FROM `pd_bolos`", {}, function(matches)
					TriggerClientEvent('erp_mdt:getAllBolos', result.source, matches)		
				end)
			elseif result.job and (result.job.name == 'ambulance') then
				exports.oxmysql:fetch("SELECT * FROM `ems_icu`", {}, function(matches)
					TriggerClientEvent('erp_mdt:getAllBolos', result.source, matches)		
				end)
			end
		end
...
AddEventHandler('erp_mdt:searchBolos', function(sentSearch)
	if sentSearch then
		TriggerEvent('echorp:getplayerfromid', source, function(result)
			if result then
				if result.job and (result.job.isPolice or result.job.name == 'doj') then
					exports.oxmysql:fetch("SELECT * FROM `pd_bolos` WHERE `id` LIKE :query OR LOWER(`title`) LIKE :query OR `plate` LIKE :query OR LOWER(`owner`) LIKE :query OR LOWER(`individual`) LIKE :query OR LOWER(`detail`) LIKE :query OR LOWER(`officersinvolved`) LIKE :query OR LOWER(`tags`) LIKE :query OR LOWER(`author`) LIKE :query", {
				...
				elseif result.job and (result.job.name == 'ambulance') then
					exports.oxmysql:fetch("SELECT * FROM `ems_icu` WHERE `id` LIKE :query OR LOWER(`title`) LIKE :query OR `plate` LIKE :query OR LOWER(`owner`) LIKE :query OR LOWER(`individual`) LIKE :query OR LOWER(`detail`) LIKE :query OR LOWER(`officersinvolved`) LIKE :query OR LOWER(`tags`) LIKE :query OR LOWER(`author`) LIKE :query", {
						query = string.lower('%'..sentSearch..'%') -- % wildcard, needed to search for all alike results
					}, function(matches)
						TriggerClientEvent('erp_mdt:getBolos', result.source, matches)
					end)
...
AddEventHandler('erp_mdt:getBoloData', function(sentId)
	if sentId then
		TriggerEvent('echorp:getplayerfromid', source, function(result)
			if result then
				if result.job and (result.job.isPolice or result.job.name == 'doj') then
					exports.oxmysql:fetch("SELECT * FROM `pd_bolos` WHERE `id` = :id LIMIT 1", {
						id = sentId
					}, function(matches)
						local data = matches[1]
						data['tags'] = json.decode(data['tags'])
				...
				elseif result.job and (result.job.name == 'ambulance') then
					exports.oxmysql:fetch("SELECT * FROM `ems_icu` WHERE `id` = :id LIMIT 1", {
						id = sentId
					}, function(matches)
						local data = matches[1]
						data['tags'] = json.decode(data['tags'])
			...
			if result then
				if result.job and (result.job.isPolice or result.job.name == 'doj') then
					...
					local function UpdateBolo()
						exports.oxmysql:execute("UPDATE pd_bolos SET `title`=:title, plate=:plate, owner=:owner, individual=:individual, detail=:detail, tags=:tags, gallery=:gallery, officersinvolved=:officersinvolved WHERE `id`=:id LIMIT 1", { 
							title = title,
							plate = plate,
							owner = owner,
							individual = individual,
				...
				elseif result.job and (result.job.name == 'ambulance') then
					...
					local function UpdateBolo()
						exports.oxmysql:execute("UPDATE `ems_icu` SET `title`=:title, plate=:plate, owner=:owner, individual=:individual, detail=:detail, tags=:tags, gallery=:gallery, officersinvolved=:officersinvolved WHERE `id`=:id LIMIT 1", { 
							title = title,
							plate = plate,
							owner = owner,
							individual = individual,
...
AddEventHandler('erp_mdt:deleteBolo', function(id)
	if id then
		TriggerEvent('echorp:getplayerfromid', source, function(result)
			if result then
				if result.job and (result.job.isPolice or result.job.name == 'doj') then
					exports.oxmysql:fetchSync("DELETE FROM `pd_bolos` WHERE id=:id", { id = id })
		...
		TriggerEvent('echorp:getplayerfromid', source, function(result)
			if result then
				if result.job and (result.job.name == 'ambulance') then
					exports.oxmysql:fetchSync("DELETE FROM `ems_icu` WHERE id=:id", { id = id })
        ...
        TriggerEvent('echorp:getplayerfromid', source, function(result)
            if result then
                if result.job and (result.job.isPolice or result.job.name == 'doj') then
                    ...
                    exports.oxmysql:fetch("SELECT id, firstname, lastname, profilepic, gender FROM `users` WHERE LOWER(`firstname`) LIKE :query OR LOWER(`lastname`) LIKE :query OR LOWER(`id`) LIKE :query OR CONCAT(LOWER(`firstname`), ' ', LOWER(`lastname`)) LIKE :query LIMIT 30", {
...
AddEventHandler('erp_mdt:getAllReports', function()
	TriggerEvent('echorp:getplayerfromid', source, function(result)
		if result then
			if result.job and (result.job.isPolice) then
				exports.oxmysql:fetch("SELECT * FROM `pd_reports` ORDER BY `id` DESC LIMIT 30", {}, function(matches)
					TriggerClientEvent('erp_mdt:getAllReports', result.source, matches)		
				end)
			elseif result.job and (result.job.name == 'ambulance') then
				exports.oxmysql:fetch("SELECT * FROM `ems_reports` ORDER BY `id` DESC LIMIT 30", {}, function(matches)
					TriggerClientEvent('erp_mdt:getAllReports', result.source, matches)		
				end)
			elseif result.job and (result.job.name == 'doj') then
				exports.oxmysql:fetch("SELECT * FROM `doj_reports` ORDER BY `id` DESC LIMIT 30", {}, function(matches)
					TriggerClientEvent('erp_mdt:getAllReports', result.source, matches)		
				end)
			end
		end
...
AddEventHandler('erp_mdt:getReportData', function(sentId)
	if sentId then
		TriggerEvent('echorp:getplayerfromid', source, function(result)
			if result then
				if result.job and result.job.isPolice then
					exports.oxmysql:fetch("SELECT * FROM `pd_reports` WHERE `id` = :id LIMIT 1", {
						id = sentId
					}, function(matches)
						local data = matches[1]
						data['tags'] = json.decode(data['tags'])
				...
				elseif result.job and (result.job.name == 'ambulance') then
					exports.oxmysql:fetch("SELECT * FROM `ems_reports` WHERE `id` = :id LIMIT 1", {
						id = sentId
					}, function(matches)
						local data = matches[1]
						data['tags'] = json.decode(data['tags'])
				...
				elseif result.job and (result.job.name == 'doj') then
					exports.oxmysql:fetch("SELECT * FROM `doj_reports` WHERE `id` = :id LIMIT 1", {
						id = sentId
					}, function(matches)
						local data = matches[1]
						data['tags'] = json.decode(data['tags'])
...
AddEventHandler('erp_mdt:searchReports', function(sentSearch)
	if sentSearch then
		TriggerEvent('echorp:getplayerfromid', source, function(result)
			if result then
				if result.job and result.job.isPolice then
					exports.oxmysql:fetch("SELECT * FROM `pd_reports` WHERE `id` LIKE :query OR LOWER(`author`) LIKE :query OR LOWER(`title`) LIKE :query OR LOWER(`type`) LIKE :query OR LOWER(`detail`) LIKE :query OR LOWER(`tags`) LIKE :query ORDER BY `id` DESC LIMIT 50", {
						query = string.lower('%'..sentSearch..'%') -- % wildcard, needed to search for all alike results
					}, function(matches)
						TriggerClientEvent('erp_mdt:getAllReports', result.source, matches)
					end)
				elseif result.job and (result.job.name == 'ambulance') then
					exports.oxmysql:fetch("SELECT * FROM `ems_reports` WHERE `id` LIKE :query OR LOWER(`author`) LIKE :query OR LOWER(`title`) LIKE :query OR LOWER(`type`) LIKE :query OR LOWER(`detail`) LIKE :query OR LOWER(`tags`) LIKE :query ORDER BY `id` DESC LIMIT 50", {
						query = string.lower('%'..sentSearch..'%') -- % wildcard, needed to search for all alike results
					}, function(matches)
						TriggerClientEvent('erp_mdt:getAllReports', result.source, matches)
					end)
				elseif result.job and (result.job.name == 'doj') then
					exports.oxmysql:fetch("SELECT * FROM `doj_reports` WHERE `id` LIKE :query OR LOWER(`author`) LIKE :query OR LOWER(`title`) LIKE :query OR LOWER(`type`) LIKE :query OR LOWER(`detail`) LIKE :query OR LOWER(`tags`) LIKE :query ORDER BY `id` DESC LIMIT 50", {
						query = string.lower('%'..sentSearch..'%') -- % wildcard, needed to search for all alike results
					}, function(matches)
						TriggerClientEvent('erp_mdt:getAllReports', result.source, matches)
					end)
		...
		TriggerEvent('echorp:getplayerfromid', source, function(result)
			if result then
				if result.job and result.job.isPolice then
					...
					local function UpdateBolo()
						exports.oxmysql:execute("UPDATE `pd_reports` SET `title`=:title, type=:type, detail=:detail, tags=:tags, gallery=:gallery, officersinvolved=:officersinvolved, civsinvolved=:civsinvolved WHERE `id`=:id LIMIT 1", { 
							title = title,
							type = reporttype,
							detail = detail,
							tags = json.encode(tags),
				...
				elseif result.job and (result.job.name == 'ambulance') then
					...
					local function UpdateBolo()
						exports.oxmysql:execute("UPDATE `ems_reports` SET `title`=:title, type=:type, detail=:detail, tags=:tags, gallery=:gallery, officersinvolved=:officersinvolved, civsinvolved=:civsinvolved WHERE `id`=:id LIMIT 1", { 
							title = title,
							type = reporttype,
							detail = detail,
							tags = json.encode(tags),
				...
				elseif result.job and (result.job.name == 'doj') then
					...
					local function UpdateBolo()
						exports.oxmysql:fetch("UPDATE `doj_reports` SET `title`=:title, type=:type, detail=:detail, tags=:tags, gallery=:gallery, officersinvolved=:officersinvolved, civsinvolved=:civsinvolved WHERE `id`=:id LIMIT 1", { 
							title = title,
							type = reporttype,
							detail = detail,
							tags = json.encode(tags),
...
local function GetImpoundStatus(vehicleid, cb)
	cb( #(exports.oxmysql:fetchSync('SELECT id FROM `impound` WHERE `vehicleid`=:vehicleid', {['vehicleid'] = vehicleid })) > 0 )
end

local function GetBoloStatus(plate, cb)
	cb(exports.oxmysql:fetchSync('SELECT id FROM `pd_bolos` WHERE LOWER(`plate`)=:plate', { plate = string.lower(plate)}))
end

local function GetOwnerName(cid, cb)
	cb(exports.oxmysql:fetchSync('SELECT firstname, lastname FROM `users` WHERE id=:cid LIMIT 1', { cid = cid}))
end

local function GetVehicleInformation(plate, cb)
	cb(exports.oxmysql:fetchSync('SELECT id, information FROM `pd_vehicleinfo` WHERE plate=:plate', { plate = plate}))
end
...
AddEventHandler('erp_mdt:searchVehicles', function(search, hash)
	if search then
		TriggerEvent('echorp:getplayerfromid', source, function(result)
			if result then
				if result.job and (result.job.isPolice or result.job.name == 'doj') then
					exports.oxmysql:fetch("SELECT id, owner, plate, vehicle, code, stolen, image FROM `owned_vehicles` WHERE LOWER(`plate`) LIKE :query OR LOWER(`vehicle`) LIKE :hash LIMIT 25", {
...
AddEventHandler('erp_mdt:getVehicleData', function(plate)
	if plate then
		TriggerEvent('echorp:getplayerfromid', source, function(result)
			if result then
				if result.job and (result.job.isPolice or result.job.name == 'doj') then
					exports.oxmysql:fetch("SELECT id, owner, plate, vehicle, code, stolen, image FROM `owned_vehicles` WHERE plate=:plate LIMIT 1", { plate = string.gsub(plate, "^%s*(.-)%s*$", "%1")}, function(vehicle)
		...
		TriggerEvent('echorp:getplayerfromid', source, function(result)
			if result then
				if result.job and (result.job.isPolice or result.job.name == 'doj') then
					...
					exports.oxmysql:fetchSync("UPDATE owned_vehicles SET `image`=:image WHERE `plate`=:plate LIMIT 1", { plate = string.gsub(plate, "^%s*(.-)%s*$", "%1"), image = imageurl })
					...
					elseif tonumber(dbid) > 0 then
						exports.oxmysql:fetchSync("UPDATE pd_vehicleinfo SET `information`=:information WHERE `plate`=:plate LIMIT 1", { plate = string.gsub(plate, "^%s*(.-)%s*$", "%1"), information = notes })
					end
				end
			end
		end)
			...
			if result then
				if result.job and (result.job.isPolice or result.job.name == 'doj') then
					...
					if type == 'code5' and status == true then
						exports.oxmysql:fetchSync("UPDATE owned_vehicles SET `code`=:code WHERE `plate`=:plate LIMIT 1", { plate = string.gsub(plate, "^%s*(.-)%s*$", "%1"), code = 5 })
					...
					elseif type == 'code5' and not status then
						exports.oxmysql:fetchSync("UPDATE owned_vehicles SET `code`=:code WHERE `plate`=:plate LIMIT 1", { plate = string.gsub(plate, "^%s*(.-)%s*$", "%1"), code = 0 })
					...
					elseif type == 'stolen' and status then
						exports.oxmysql:fetchSync("UPDATE owned_vehicles SET `stolen`=:stolen WHERE `plate`=:plate LIMIT 1", { plate = string.gsub(plate, "^%s*(.-)%s*$", "%1"), stolen = 1 })
					...
					elseif type == 'stolen' and not status then
						exports.oxmysql:fetchSync("UPDATE owned_vehicles SET `stolen`=:stolen WHERE `plate`=:plate LIMIT 1", { plate = string.gsub(plate, "^%s*(.-)%s*$", "%1"), stolen = 0 })
	...
	TriggerEvent('echorp:getplayerfromid', source, function(result)
		if result then
			if LogPerms[result.job.name][result.job.grade] then
				exports.oxmysql:fetch('SELECT * FROM pd_logs ORDER BY `id` DESC LIMIT 250', {}, function(infoResult)
...
local function IsCidFelon(sentCid, cb)
	if sentCid then
		exports.oxmysql:fetch('SELECT charges FROM pd_convictions WHERE cid=:cid', { cid = sentCid }, function(convictions)
			local Charges = {}
			for i=1, #convictions do
	...
	TriggerEvent('echorp:getplayerfromid', source, function(result)
		...
		if player then
			if player.job.name == "ambulance" and player.job.duty == 0 then
                ...
                if mzDist > 100 then TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'You must be at Mount Zonah to clock in!!', length = 5000 }) TriggerClientEvent('erp_mdt:exitMDT',source) return end
            end
			...
			TriggerEvent('echorp:notifyDuty', player, "toggle")
		end
	end)
end)
...
local function fuckme(cid, incident, data, cb)
	cb(exports.oxmysql:fetchSync('SELECT * FROM pd_convictions WHERE cid=:cid AND linkedincident=:linkedincident', { cid = cid, linkedincident = id }), data)
end
...
AddEventHandler('erp_mdt:handleExistingConvictions', function(data, incidentid, time)
	exports.oxmysql:fetch('SELECT * FROM pd_convictions WHERE cid=:cid AND linkedincident=:linkedincident', {
		cid = data['Cid'],
...
AddEventHandler('erp_mdt:setWaypoint:unit', function(cid)
	...
	TriggerEvent('echorp:getplayerfromid', source, function(me)
		local player = exports['echorp']:GetPlayerFromCid(cid)
		if player then
			TriggerClientEvent('mythic_notify:client:SendAlert', player.source, { type = 'inform', text = me['fullname']..' set a waypoint on you!', length = 5000 })
...
AddEventHandler('erp_mdt:sendMessage', function(message, time)
	if message and time then
		TriggerEvent('echorp:getplayerfromid', source, function(player)
			if player then
				exports.oxmysql:fetch("SELECT id, profilepic, gender FROM `users` WHERE id=:id LIMIT 1", {
		...
		if result then
			if result.job.isPolice or result.job.name == 'ambulance' or result.job.name == 'doj' then
				...
				if tgtPlayer then
					...
					TriggerClientEvent('mythic_notify:client:SendAlert', result['source'], { type = 'success', text = 'Radio updated.', length = 5000 })
				end
			end
		end
	end)
			...
			if player.job.isPolice then	
				if sentInfo and type(sentInfo) == 'table' then
					...
					if (plate and linkedreport and fee and time) then
						exports.oxmysql:fetch("SELECT id, plate FROM `owned_vehicles` WHERE plate=:plate LIMIT 1", { plate = string.gsub(plate, "^%s*(.-)%s*$", "%1")}, function(vehicle)
							if vehicle and vehicle[1] then
		...
		if player then
			if player.job.isPolice then	
				exports.oxmysql:fetch("SELECT id, plate FROM `owned_vehicles` WHERE plate=:plate LIMIT 1", { plate = string.gsub(plate, "^%s*(.-)%s*$", "%1")}, function(vehicle)
					if vehicle and vehicle[1] then
						local data = vehicle[1]
						exports.oxmysql:fetchSync("DELETE FROM `impound` WHERE vehicleid=:vehicleid", { vehicleid = data['id'] })
					end
				end)
			end
		end
		...
		if player then
			if player.job.isPolice then	
				exports.oxmysql:fetch("SELECT id, plate FROM `owned_vehicles` WHERE plate=:plate LIMIT 1", { plate = string.gsub(plate, "^%s*(.-)%s*$", "%1")}, function(vehicle)
					if vehicle and vehicle[1] then
						local data = vehicle[1]
						exports.oxmysql:fetch("SELECT * FROM `impound` WHERE vehicleid=:vehicleid LIMIT 1", { vehicleid = data['id'] }, function(impoundinfo)
